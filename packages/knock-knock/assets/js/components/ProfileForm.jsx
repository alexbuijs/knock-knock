...
const ProfileForm = ({ data, setUserImage }) => {
    ...
    return axios({
      ...
      .catch((error) => {
        if (error.response) {
          setAlert({
            type: "danger",
            message: `Er ging helaas iets mis (code ${error.response.status})`,
          });
        } else {
  ...
  return (
    <form onSubmit={handleSubmit} ref={formRef}>
      {alert && (
        <Alert type={alert.type} close={() => setAlert(false)}>
          {alert.message}
        </Alert>
      )}
      ...
      <div className="mt-2">
        <label htmlFor="phone" className="form-label">
          E-mailadres <small>(Let op: wordt niet gecontroleerd)</small>
        </label>
        <input
      ...
      <div className="mt-2">
        <label className="form-label" htmlFor="photo">
          ...
          <small>(max. {data.uploadMaxFilesize}B)</small> en klik op opslaan.
        </label>
        <input
          onChange={handleChange}
          onBlur={handleBlur}
